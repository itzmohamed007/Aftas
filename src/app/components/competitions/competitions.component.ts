...
export class CompetitionsComponent  implements OnInit {
  ...
  constructor(private competitionService:CompetitionService, private rankingService:RankingService, private memberService:MemberService) {}
  ngOnInit(): void {
  ...
  displayLinkingDialog(competition:Competition) {
    ...
    this.filteredMembers = this.members.filter(member => {
      return !competition.rankings.some(ranking => ranking.member.num == member.num)
    })
  ...
  linkMember() {
    ...
    this.rankingService.linkMember(this.memberToLink, this.competitionToLink).subscribe({
      next: ranking => {
        console.log(ranking);
        this.loadCompetitions();
